...
const ACHIEVEMENTS = [
  ...
  {
    slug: "community-star",
    check: (s) => s.totalUpvotes - s.totalDownvotes >= 50,
  },
...
async function computeScore(userId) {
  ...
  const netAnswerUpvotes = answers.reduce(
    (s, a) => s + Math.max(0, a.votesCount),
    0,
  );
  const netAnswerDownvotes = answers.reduce(
    (s, a) => s + Math.max(0, -a.votesCount),
    0,
  );
  ...
  const consistencyRatio =
    totalPosts > 0 ? Math.min(answers.length / totalPosts, 1) : 0;
  ...
  const toCreate = ACHIEVEMENTS.filter(
    (a) => !earnedSet.has(a.slug) && a.check(stats),
...
async function main() {
  ...
  console.log("🧹 Wiping old database records to ensure a clean slate...");
  ...
  await prisma.user.deleteMany({
    where: { clerkUserId: { startsWith: "fake_clerk" } },
  });
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const title =
      threadTitles[i % threadTitles.length] +
      (i > 9 ? ` (Part ${Math.floor(i / 10) + 1})` : "");
  ...
  for (const thread of createdThreads) {
    const numReplies = Math.floor(Math.random() * 25) + 3; // 3 to 27 replies per thread

    for (let i = 0; i < numReplies; i++) {
      // Pick random author (not the thread author to allow verifications realistically)
      const availableAuthors = mockUsers.filter(
        (u) => u.id !== thread.authorId,
      );
      const author =
        availableAuthors[Math.floor(Math.random() * availableAuthors.length)];

      const isAnswer = Math.random() > 0.4; // 60% chance of being an Answer
      ...
      await prisma.reply.create({
        data: {
          ...
          verifiedBy: verifiedAt
            ? { connect: { id: thread.authorId } }
            : undefined,
          votesCount,
          votes: { create: voteData },
          createdAt: new Date(
            thread.createdAt.getTime() + Math.random() * 86400000,
          ), // Up to 1 day after thread
        },
      });
      replyCount++;
    }
  ...
  console.log(
    "Calculating ClarityScores & awarding achievements... (this might take a minute)",
  );
