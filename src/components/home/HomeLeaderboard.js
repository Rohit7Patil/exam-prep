    ...
    <section className="py-16 sm:py-24 border-t border-border/40 overflow-hidden relative">
      ...
      <div className="mx-auto max-w-4xl px-4 relative z-10 text-center">
        ...
        <p className="mb-12 text-muted-foreground">
          Meet the most helpful minds on ExamPrep India. Can you beat their
          ClarityScore?
        </p>
                ...
                <div className="relative">
                  {s.user.avatarUrl ? (
                    <img
                      ...
                      alt={s.user.username}
                      className={`rounded-full object-cover ring-4 ring-background ${isFirst ? "h-20 w-20" : "h-14 w-14"}`}
                    />
                  ) : (
                    <div
                      className={`rounded-full flex items-center justify-center font-black ${color} bg-current/20 ring-4 ring-background ${isFirst ? "h-20 w-20 text-3xl" : "h-14 w-14 text-xl"}`}
                    >
                      {(s.user.username || "A")[0].toUpperCase()}
                    </div>
                  )}
                  <span className="absolute -bottom-2 -right-2 text-2xl drop-shadow-md bg-background rounded-full">
                    {medal}
                  </span>
                </div>

                <div className="text-center mt-2">
                  <p className="text-sm font-bold text-foreground truncate max-w-[100px] sm:max-w-full">
                    {s.user.username}
                  </p>
                  <p className={`text-xl font-black mt-1 ${color}`}>
                    ⚡{Math.round(s.clarityScore)}
                  </p>
                </div>
              </Link>
            );
